...
- **`Program.cs`** — the entry point. Registers Blazor server-side rendering
  ...
  — this tells the Blazor router to also discover `@page` routes declared in
  `ResumeApp.Shared`, which is how `/experience`, `/education`, etc. get served even though
  they're not defined in this project.
